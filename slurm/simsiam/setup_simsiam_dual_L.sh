#!/bin/bash  
#SBATCH --job-name=SimSiam_pollen_L           # Job name shown in queue
#SBATCH --partition=h200                      # Partition / GPU queue
#SBATCH --nodes=1                             # Number of nodes
#SBATCH --ntasks-per-node=1                   # Tasks per node
#SBATCH --gres=gpu:1                          # Number of GPUs
#SBATCH --cpus-per-task=16                    # CPU cores per task
#SBATCH --mem=20G                             # RAM per node
#SBATCH --time=2-4:00:00                      # Max runtime (D-HH:MM:SS)
#SBATCH --hint=nomultithread                  # Disable hyperthreading
#SBATCH --output=SimSiam_L_%j.out             # Standard output log
#SBATCH --error=SimSiam_L_%j.err              # Error log
...
CONFIG_PATH=/app/config/slurm_simsiam_dual_config_L.yaml
